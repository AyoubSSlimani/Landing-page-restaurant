...
document.addEventListener("scroll", (e)=>{
    const percentageScroll = window.scrollY / (document.body.offsetHeight - window.innerHeight); // Position verticale du scroll
    console.log(percentageScroll.toFixed(2));
    if (percentageScroll > 0.03) title.style.opacity = "0";
    else if (scrollY < 0.03) title.style.opacity = "1";
    ...
    // NAV
    if (window.scrollY < scrollPosition) nav.style.top = 0;
...
burgerbutton.addEventListener("click", (e)=>{
    ...
    if (isActive) burgerButtonImg.style.transform = "";
    else burgerButtonImg.style.transform = "rotate(360deg)";
...
burgerButtons.forEach((btn)=>btn.addEventListener("click", (e)=>{
        burgerButtonImg.classList.toggle("fa-burger");
        burgerButtonImg.classList.toggle("fa-minus");
        const isVisible = !burgerMenu.classList.contains("invisible");
        if (isVisible) {
